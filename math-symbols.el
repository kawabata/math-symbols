...
;;   "\int g(x^2)dx = \pi e^(i𝑥)" → "∫ 𝑔(𝑥²)𝑑𝑥 = 𝜋 𝑒^(𝑖𝑥)"  
...
(defvar math-symbols-style-table
  #s(hash-table
     test equal data
     ("BOLD" 
      #s(hash-table 
...
(defvar math-symbols-style-names
  (let (names) (maphash (lambda (k v) (push k names)) math-symbols-style-table) names))

(defvar math-symbols-superscript-regexp
  (let ((table (gethash "SUPERSCRIPT" math-symbols-style-table))
        result)
...
(defvar math-symbols-subscript-regexp
  (let ((table (gethash "SUBSCRIPT" math-symbols-style-table))
        result)
    (maphash (lambda (k v) (push k result)) table)
    (regexp-opt (mapcar 'char-to-string result))))
...
(defun math-symbols-style-table (style)
  (or (gethash style math-symbols-style-table)
...
(defun math-superscript-string (string)
  (with-temp-buffer
    (insert string)
    (math-superscript-region (point-min) (point-max))
    (buffer-string)))
...
(defun math-subscript-string (string)
  (with-temp-buffer
    (insert string)
    (math-subscript-region (point-min) (point-max))
    (buffer-string)))
...
(defun math-symbols-super/subscript-from-tex-region (from to)
  (save-excursion
    (save-restriction
      (narrow-to-region from to)
      (goto-char (point-min))
      (while (re-search-forward (concat "\\^{\\(" math-symbols-superscript-regexp "+\\)}") nil t)
        (replace-match (math-superscript-string (buffer-substring (match-beginning 1) (match-end 1)))))
      (goto-char (point-min))
      (while (re-search-forward (concat "\\^\\(" math-symbols-superscript-regexp "\\)") nil t)
        (replace-match (math-superscript-string (buffer-substring (match-beginning 1) (match-end 1)))))
      (goto-char (point-min))
      (while (re-search-forward (concat "_{\\(" math-symbols-subscript-regexp "+\\)}") nil t)
        (replace-match (math-subscript-string (buffer-substring (match-beginning 1) (match-end 1)))))
      (goto-char (point-min))
      (while (re-search-forward (concat "_\\(" math-symbols-subscript-regexp "\\)") nil t)
        (replace-match (math-subscript-string (buffer-substring (match-beginning 1) (match-end 1))))))))
...
(defun math-symbols-super/subscript-to-tex-region (from to)
  (save-excursion
    (save-restriction
      (narrow-to-region from to)
      (goto-char (point-min))
      (while (re-search-forward (concat math-symbols-superscript-to-regexp "+") nil t)
        (let ((length (length (match-string 0)))
              (chars (save-match-data (ucs-normalize-NFKC-string 
                                       (buffer-substring (match-beginning 0) (match-end 0))))))
          (replace-match (concat "^" (if (< 1 length) "{") chars (if (< 1 length) "}")))))
      (goto-char (point-min))
      (while (re-search-forward (concat math-symbols-subscript-to-regexp "+") nil t)
        (let ((length (length (match-string 0)))
              (chars (save-match-data (ucs-normalize-NFKC-string 
                                       (buffer-substring (match-beginning 0) (match-end 0))))))
          (replace-match (concat "_" (if (< 1 length) "{") chars (if (< 1 length) "}"))))))))
...
For example, '\Phi' will be converted to '𝛷'."
  ...
  (save-excursion
    (save-restriction
      ...
      (math-italic-region (point-min) (point-max)))))
